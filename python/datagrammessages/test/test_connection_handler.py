...
class ConnectionHandlerTestCase(unittest.TestCase):
    ...
    def test_send(self):
        s = conn.ConnectionHandler()
        s.sock_send = Mock()
        s.send('test', 'foo')
        s.sock_send.assert_called_with(msgpack.packb(['test', 'foo']))
    ...
    def test_handle_datagram(self):
        ...
        s.handle_datagram(conn.encode('req', ['test', 1234, 'args']))
        s._handle_service_req.assert_called_once_with(['test', 1234, 'args'])
        s.handle_datagram(conn.encode('res', ['test', 1234, 'args']))
        s._handle_service_res.assert_called_once_with(['test', 1234, 'args'])
        s.handle_datagram(conn.encode('test', 'foo'))
    ...
    def test_default_message_handler(self):
        ...
        s.handle_datagram(conn.encode('foo', 123))
    ...
    def test_handle_service(self):
        ...
        s.handle_datagram(conn.encode('req', ['test', 1234, '6*9']))
        handler.assert_called_once_with('6*9')
        expected_result = conn.encode('res', ['test', 1234, 42])
        s.sock_send.assert_called_once_with(expected_result)
    ...
    def test_service_call(self, event_wait_mock):
        ...
        s.sock_send.assert_called_once_with(expected_req)
